...
You'll add new data to the class, and customize the give_raise() method from Chapter 1 to increase the manager's raise amount by a bonus percentage whenever they are given a raise.
...
class Manager(Employee):
    ...
    The Manager constructor accepts name, salary (default 50000), and project (default None).
    It calls the parent Employee constructor with name and salary using Employee.__init__(self, name, salary),
    and assigns the project attribute to self.project.
    This allows Manager to inherit Employee's attributes while adding its own.
    """

    """
    Explanation for Step 2:
    The give_raise method in Manager overrides the parent's method. It accepts amount and bonus (default 1.05),
    multiplies amount by bonus to apply a 5% bonus by default, then calls Employee.give_raise(self, amount)
    to update the salary. This customizes the raise for managers.
    """
